...
unsigned long decodeLongLE(unsigned char* p_buffer, unsigned long p_offset) {
	return
		p_buffer[p_offset] |
		(p_buffer[p_offset + 1] << 8) |
		(p_buffer[p_offset + 2] << 16) |
...
unsigned long decodeIntBEGlobal(__global unsigned char* p_buffer, unsigned long p_offset) {
	return
...
unsigned long decodeLongBE(unsigned char* p_buffer, unsigned long p_offset) {
	return
		(p_buffer[p_offset] << 56) |
		(p_buffer[p_offset + 1] << 48) |
		(p_buffer[p_offset + 2] << 40) |
		(p_buffer[p_offset + 3] << 32) |
		(p_buffer[p_offset + 4] << 24) |
		(p_buffer[p_offset + 5] << 16) |
		(p_buffer[p_offset + 6] << 8) |
		p_buffer[p_offset + 7];
}
